...
export default async function ActualitePage({
  ...
  try {
    ...
    articles = activeFilter === 'Tous'
      ? all
      : all.filter((a) => a.categorie === activeFilter);
  } catch {}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-wrap gap-2 mb-10">
          {categories.map((cat) => (
            <Link
              key={cat}
              href={cat === 'Tous' ? '/actualite' : `/actualite?categorie=${encodeURIComponent(cat)}`}
        ...
        {articles.length === 0 ? (
          <div className="text-center py-20 text-[#8a96b8]">
            <p className="text-lg">Aucun article disponible pour l&apos;instant.</p>
          </div>
        ) : (
              ...
              <Link key={article.id} href={`/actualite/${article.slug}`} className="group">
                <article className="rounded-[10px] overflow-hidden bg-[#1e2c56] border border-[rgba(232,213,163,0.08)] hover:border-[rgba(232,213,163,0.2)] hover:-translate-y-1 transition-all h-full flex flex-col">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <p className="text-xs text-[#8a96b8] mb-2">{formatDate(article.created_at)}</p>
